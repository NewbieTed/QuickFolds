...
import { Point3D, createPoint3D, distance } from '../../geometry/Point';
	...
	if (input.getIsPickPointButtonPressed()) {
		...
		if (foldButtonState.state.step === 1) {
			...
			if (intersects.length > 0) {
				...
				console.log("Current fold state after update:", currentState);
		...
		if (input.getFoldButtonState().state.step === 4) {
			...
			// Add debug logging
			console.log("Fold angle state:", foldAngleState);
			console.log("Fold button state:", foldButtonState);
			console.log("wtf");
			...
			if (foldButtonState.state.point1Id !== -1n && 
				...
				console.log("hahahah", foldButtonState.state.point1Id);
				console.log("FAEID", foldButtonState.state.faceId);
				// First create the split
				const result = await createANewFoldBySplitting(
					foldButtonState.state.point1Id,
					foldButtonState.state.point2Id,
					foldButtonState.state.faceId,
					foldButtonState.state.stationaryPoint,
					foldAngleState.angle
				);
...
function projectAndCreateEdgePoint(
	...
	// TODO: test the threshold, paper 6*6, paper thickness : 0.05
    if (distance(projectedPoint, point) < 0.1) {
...
/**
 * Handles the confirmation of a fold operation
 * @param angle The angle value for the fold
 */
// export async function confirmButtonPressed(angle: bigint) {
